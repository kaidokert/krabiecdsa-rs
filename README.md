...
`no_std`, no allocator, no panics. RustCrypto [signature](https://crates.io/crates/signature) traits are supported both ways: `PrehashVerifier`/`DigestVerifier` for verification, and `PrehashSigner`/`RandomizedPrehashSigner`/`DigestSigner`/`Keypair` for constant-time signing.
