...
`no_std`, no allocator, no panics.

Only signature verification is implemented right now, as private key handling
requires constant-time operations, a trustworthy nonce derivation and secure
storage for the private key. RustCrypto
[signature](https://crates.io/crates/signature) traits are supported through
`PrehashVerifier`.

Verification is tested against Project Wycheproof adversarial vectors and
openssl-produced signatures.
